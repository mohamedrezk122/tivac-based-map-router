#include "./gps.h"
#define out(x) printf("%c", x);
// dummy until implemented
_Bool f = 1, stop = 0;
int j = 0;
char send[] = "$GPRMC,210230,A,3855.4487,N,09446.0071,W,0.0,076.2,130495,003.8,E*69";
char gp[100];
...
void params(char c)
{
    ...
    if (c == 'A')
    {
        if (values[6][0] == '1')
        {
            ...
            fprintf(f, "latit, lon, altit\n");
            fprintf(f, "%lf, %lf, %lf", latit, lon, altit);
        }
        else
            f = 0;
    }
